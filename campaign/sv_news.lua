...
function News.Initialize()
    ...
    Convergence.Events.Subscribe(
        ...
        function(event)
            local result = event.payload.result or {}
            local planet = Convergence.PlanetService.Get(
                result.planetID
            )

            News.Publish({
                headline = result.winner == "friendly"
                    and "COALITION WINS ORBITAL ENGAGEMENT"
                    or result.winner == "enemy"
                        and "HOSTILE FLEET SECURES ORBITAL ADVANTAGE"
                        or "ORBITAL BATTLE ENDS IN STALEMATE",
                subheadline = planet
                    and planet:GetName()
                    or tostring(result.planetID),
                body = string.format(
                    "Friendly strength %.0f, hostile strength %.0f after the engagement.",
                    tonumber(result.friendlyAfter) or 0,
                    tonumber(result.enemyAfter) or 0
                ),
                category = "fleet_battle",
                severity = result.winner == "friendly"
                    and "success"
                    or result.winner == "enemy"
                        and "danger"
                        or "warning",
