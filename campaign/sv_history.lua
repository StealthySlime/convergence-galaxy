...
function History.BindEvents()
    ...
    subscribe("fleet.battle.resolved", function(event)
        local result = event.payload.result or {}

        History.Record({
            category = "fleet_battle",
            eventName = event.name,
            title = "Orbital Battle Resolved",
            summary = string.format(
                "%s victory at %s. Friendly %.0f, enemy %.0f strength remaining.",
                string.upper(tostring(result.winner or "draw")),
                tostring(result.planetID or "unknown"),
